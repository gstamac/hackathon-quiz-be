...
describe('Testing Answers', () => {
  ...
  describe('[POST] /games/:game_id/answers?question_id=<question_id>&answer_id=<answer_id>&participant=<participant>', () => {
    ...
    it('response statusCode 201 / created', async () => {
      await request.post(`/games/${game.id}/answers?question_id=test-game-q2&answer_id=test-game-q2-a2&participant=participant1`).send().expect(201)
    })
    ...
    it('response statusCode 400 when question was not found', async () => {
      await request
        .post(`/games/${game.id}/answers?question_id=wrong-question&answer_id=test-game-q2-a2&participant=participant1`)
        .send()
        .expect(400)
    ...
    it('response statusCode 400 when answer was not found', async () => {
      await request
        .post(`/games/${game.id}/answers?question_id=test-game-q2&answer_id=wrong-answer&participant=participant1`)
        .send()
        .expect(400)
    ...
    it('response statusCode 400 when question was already answered correctly', async () => {
      await request.post(`/games/${game.id}/answers?question_id=test-game-q1&answer_id=test-game-q1-a3&participant=participant2`)
      await request
        .post(`/games/${game.id}/answers?question_id=test-game-q1&answer_id=test-game-q1-a2&participant=participant1`)
        .send()
        .expect(400)
    ...
    it('response statusCode 400 when question was already answered by me', async () => {
      await request.post(`/games/${game.id}/answers?question_id=test-game-q2&answer_id=test-game-q2-a1&participant=participant2`)
      await request
        .post(`/games/${game.id}/answers?question_id=test-game-q2&answer_id=test-game-q2-a2&participant=participant2`)
        .send()
        .expect(400)
